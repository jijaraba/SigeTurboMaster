                            ...
                            <section>
                                <h2>
                                    @if($payment->method == 'agreement')
                                        {{ money($payment->realValue) }} <span>(SALDO PENDIENTE: {{ money($payment->value2 -  $payment->realValue)}})</span>
                                    @else
                                        {{ money($payment->realValue) }}
                                    @endif
                                </h2>
                                <p class="observation">{{ $payment->observation1 }}</p>
                                <sige-turbo-payment-result-transaction payment="payment"></sige-turbo-payment-result-transaction>
                            </section>
                            <footer>
