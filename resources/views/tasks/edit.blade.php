    ...
    <section ng-controller="TaskUpdateController" ng-init="init({{ $task->idtask }})">
        <section class="grid-100">
            <section class="sige-contained">
                <a href="{{ URL::route('formation.tasks.index',['sort' => $sort, 'order' => $order, 'page' => $page, 'subject' => $subject])}}" class="btn btn-transparent"><i class="fa fa-arrow-left"></i>{{ Lang::get('sige.Back') }}</a>
                <section class="sige-task-new">
                    ...
                    <form ng-submit="Update()" name="taskForm" id="taskForm" method="post">
                            ...
                            <ul class="display-horizontal col-100">
                                <li class="col-100 gutter-5" ng-if="task.status != 1">
                                    <label for="draft" class="draft">
                                        <input type="radio" name="status" id="draft" value="0" ng-model="task.status" checked="true">
                                        <span>Draft</span>
                                    </label>
                                    <label for="approved">
                                        <input type="radio" name="status" id="approved" value="1" ng-model="task.status">
                                        <span>Approve</span>
                                    </label>
                                </li>
                        ...
                        <fieldset>
                            <sige-turbo-academic-embedded year="@{{task.year}}" period="@{{task.period}}" group="@{{ task.group }}" subject="@{{ task.subject }}" nivel="@{{ task.nivel }}" academic="academic"></sige-turbo-academic-embedded>
                        </fieldset>
                        <fieldset>
                            <ul class="display-horizontal col-100">
                                <li class="col-20 gutter-5">
                                    <label class="select-arrow" for="idtasktype">
                                        <select id="idtasktype" name="idtasktype" ng-model="task.type" ng-options="tasktype.idtasktype as tasktype.name for tasktype in tasktypes" required></select>
                                    </label>
                                </li>
                        ...
                        <fieldset>
                            <section class="file-list" ng-if="taskFiles" style="padding-top: 5px">
                                ...
                                <ul class="display-horizontal col-100 progress @{{ file.status | fileStatus }} @{{ file.deleted | fileDeleted }}" data-ng-repeat="file in task.taskfiles" ng-if="!file.deleted">
                                    <li class="col-05 gutter-5 icon fa fa-cloud-download"></li>
                                    <li class="col-75 gutter-5 name"><a target="_blank" href="@{{ assets }}/task/@{{ file.file }}">@{{file.realname}}</a></li>
                        ...
                        <fieldset>
                            <ul class="display-horizontal col-100">
                                <li class="col-100">
                                    <button id='hiddenButton' type="submit" class="btn btn-aquamarine">{{ Lang::get('sige.Save') }}</button>
                                </li>
                            </ul>
                        </fieldset>
                    </form>
