...
angular.module('Formation.directives', [])
        ...
        return {
            ...
            link: function (scope) {
                ...
                scope.save = function () {
                    ...
                    if (scope.academic['idcontract']) {//ESTA CONDICION CON EL PARSE JASON ES PORQUE LLEGA CONVERTIDO COMO0 STRING
                ...
                $scope.dialogsforms = function ($object) {
                    ...
                    ngDialog.open({
                        ...
                        controller: ['$scope', function ($scope) {
                            ...
                            /*$scope.validateconsecutivo = function () {
                             alert($scope.consecutiveactual);
                             if(($filter('getByPropertyAnd')({'consecutive': $scope.indicator.consecutive}, angular.copy($scope.achievements[0].indicators)) === null) || ($scope.consecutiveactual == $scope.indicator.consecutive)){
                             alert("Soy Valido");
                             }else{
                             alert("No puede usarse este consecutivo");
                             }
                             }*/
                        }]
                    });
                    //angular.element('.ngdialog-content').css('width', '100px');
                };
    ...
    .directive('sigeTurboMonitoringTypesIndicators', ['$log', function ($log) {
        return {
            restrict: 'AE',
    ...
    .directive('sigeTurboMonitoringType', ['$log', 'Monitoringtype', 'Monitoring', function ($log, Monitoringtype, Monitoring) {
        return {
            restrict: 'AE',
    ...
    .directive('sigeTurboSubjectsbyarea', ['$log', 'ngDialog','$filter', function ($log, ngDialog,$filter) {
        return {
            ...
            controller: ['$scope', function ($scope) {
                ...
                //if($scope.idarea == 1)alert(JSON.stringify($scope.subjectbyareas));
                $scope.editDialog = function(registry,$controller){
                   angular.element(document.getElementById("AcademicManagementController")).scope().$$childTail.$parent.edit(registry,$controller);//Se llama de esta manera porque si declaro el controlador AcademicManagementController en la directiva me hace el numero de peticiones a la Base de Datos tanto como se llame la directiva
                }
                $scope.deleteregistry = function ($id) {
    ...
    .directive('sigeTurboMonitoringPendingsByStudents', ['$log', '$filter', 'ASSETS_SERVER', function ($log,$filter, ASSETS_SERVER) {
        return {
            ...
            link: function (scope, element, attrs) {
                scope.$watch('registries', function (newRegistries,oldRegistries) {
                    if ((newRegistries != undefined && newRegistries != "") && (typeof newRegistries == "object")) {
                       ...
                       scope.resumen = {
                        ...
                        scope.$watch('teacherselect', function (newTeacher,oldTeacher) {
                            scope.elementsearch = {id:'teacher',value : newTeacher,searchvalue : newTeacher}; 
                        });

                        scope.$watch('studentselect', function (newStudent,oldStudent) {
                ...
                scope.$watch('elementsearch', function (newElementSearch,oldElementSearch) {
                    ...
                    if(newElementSearch.value != undefined){
                        switch (newElementSearch.id) {
                            ...
                            case 'teacher':
                                    scope.summaries = [];
                                    angular.forEach(angular.copy(scope.registries), function(subarray,indexofarray) {
    ...
    .directive('sigeTurboCandidates',['Vote', function (Vote) {
                ...
                $scope.send = function ($iduser) {
                     Vote.save({
                        ...
                        iduser:$iduser
                            ...
                            function (result) {
                                //Esconder Contenedor de Personero
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                scope.contractsname = function ($array) {
                    //alert("Soy El arreglo de usuario "+JSON.stringify(scope.users.length)+"\n Soy El arreglo de contractoriginals "+JSON.stringify($array.length)+"\n Soy El arreglo de contracts "+JSON.stringify($array.length)+" \n Soy el arreglo de Groups "+JSON.stringify(scope.groups.length)+"\n Soy el arreglo de Subjects "+JSON.stringify(scope.subjects.length)+"\n Soy el arreglo de Nivels "+JSON.stringify(scope.nivelsoriginal.length));
                    /*alert(JSON.stringify(scope.users));
                     alert(JSON.stringify(scope.contracts));*/
                    ...
                    if ((newGroups != undefined && newGroups != "") && (typeof newGroups == "object")) {
                        scope.$watch('academic.idgroup', function (newGroup, oldGroup) {
                            if (newGroup !== 'Loading ...' && newGroup !== oldGroup) {
                                //alert(JSON.stringify(scope.academic));
                                ...
                                //alert(JSON.stringify(scope.academic));
                                //alert('En Groups es: idgroup : '+scope.academic.idgroup+' idsubject : '+scope.academic.idsubject+' idnivel : '+scope.academic.idnivel);
                                scope.contracts = scope.contractsname($filter('getByPropiertyAndArray')(scope.nonulos(scope.academic), angular.copy(scope.contractoriginals)));
                                //alert(JSON.stringify(scope.contracts));
                            }
                        });
                ...
                /*scope.$watch('contractoriginals', function (newContracts) {
                 if ((newContracts != undefined &&  newContracts != "") && (typeof newContracts == "object")) {
                 scope.contracts = scope.contractsname($filter('getByPropiertyAndArray')(scope.nonulos(scope.academic), angular.copy(scope.contractoriginals)));
                 }
                 });*/
                    ...
                    if ((newSubjects != undefined && newSubjects != "") && (typeof newSubjects == "object")) {
                        scope.$watch('academic.idsubject', function (newSubject, oldSubject) {
                            if (newSubject !== 'Loading ...' && newSubject !== oldSubject) {
                                ...
                                scope.academic.idnivel = 'Loading ...';
                                scope.academic.iduser = undefined;
                                //alert('En Subjects es idgroup : '+scope.academic.idgroup+'idsubject : '+scope.academic.idsubject+'idnivel : '+scope.academic.idnivel);
                                //alert(JSON.stringify(scope.nonulos(scope.academic)));
                                scope.contracts = scope.contractsname($filter('getByPropiertyAndArray')(scope.nonulos(scope.academic), angular.copy(scope.contractoriginals)));
                                //alert(JSON.stringify(scope.contracts));
                            }
                        });
                    ...
                    if ((newNivels != undefined && newNivels != "") && (typeof newNivels == "object")) {
                        scope.$watch('academic.idnivel', function (newNivel, oldNivel) {
                            if (newNivel !== 'Loading ...' && newNivel !== oldNivel) {
                                //alert('En Nivels es: idgroup : '+scope.academic.idgroup+' idsubject : '+scope.academic.idsubject+' idnivel : '+scope.academic.idnivel);
                                scope.contracts = scope.contractsname($filter('getByPropiertyAndArray')(scope.nonulos(scope.academic), angular.copy(scope.contractoriginals)));
                                //alert(JSON.stringify(scope.contracts));
                                scope.academic.iduser = undefined;
                                scope.academic.idcontract = undefined;
