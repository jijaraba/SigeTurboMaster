...
class MonitoringRepository implements MonitoringRepositoryInterface
{
    ...
    {
        return Monitoring::select(DB::raw('ANY_VALUE(monitorings.idyear) AS idyear'), DB::raw('ANY_VALUE(monitorings.idperiod) AS idperiod'), DB::raw('ANY_VALUE(monitorings.idgroup) AS idgroup'), DB::raw('ANY_VALUE(monitorings.idsubject) AS idsubject'), DB::raw('ANY_VALUE(monitorings.idnivel) AS idnivel'), DB::raw('ANY_VALUE(iduser) AS iduser'), 'monitoringcategories.name AS category', DB::raw('ANY_VALUE(monitoringcategorybyyears.percent) AS percent)', DB::raw("CONCAT('[',GROUP_CONCAT(CONCAT('{','\"monitoringtype\"',':','\"',REPLACE(monitoringtypes.name, '\"' , \"'\" ),'\"',',\"rating\"',':','\"',rating,'\"','}') ORDER BY monitoringtypes.name ),']') AS details"), DB::raw("ROUND(AVG(monitorings.rating) * monitoringcategorybyyears.percent, 2) AS average"))
