...
class MonitoringRepository implements MonitoringRepositoryInterface
{
    ...
    {
        return Monitoring::select(DB::raw("ANY_VALUE(idmonitoringtype) AS idmonitoringtype, ANY_VALUE(idmonitoring) AS idmonitoring,CASE WHEN (ANY_VALUE(rating) BETWEEN 0.00 AND 2.99) THEN 'DP' ELSE CASE WHEN (ANY_VALUE(rating) BETWEEN 3.00 AND 3.70) THEN 'DB' ELSE CASE WHEN (ANY_VALUE(rating) BETWEEN 3.71 AND 4.30) THEN 'DA' ELSE CASE WHEN (ANY_VALUE(rating) BETWEEN 4.31 AND 5.00) THEN 'DS' END END END END label, CASE WHEN (ANY_VALUE(rating) BETWEEN 0.00 AND 2.99) THEN '#ED5565' ELSE CASE WHEN (ANY_VALUE(rating) BETWEEN 3.00 AND 3.70) THEN '#FC6E51' ELSE CASE WHEN (ANY_VALUE(rating) BETWEEN 3.71 AND 4.30) THEN '#2f9da3' ELSE CASE WHEN (ANY_VALUE(rating) BETWEEN 4.31 AND 5.00) THEN '#A0D468' END END END END color,CASE WHEN (ANY_VALUE(rating) BETWEEN 0.00 AND 2.99) THEN '#DA4453' ELSE CASE WHEN (ANY_VALUE(rating) BETWEEN 3.00 AND 3.70) THEN '#E9573F' ELSE CASE WHEN (ANY_VALUE(rating) BETWEEN 3.71 AND 4.30) THEN '#3ababa' ELSE CASE WHEN (ANY_VALUE(rating) BETWEEN 4.31 AND 5.00) THEN '#8CC152' END END END END highlight,COUNT(*) value"))
    ...
    {
        return Monitoring::select(DB::raw('ANY_VALUE(monitorings.idyear) AS idyear'), DB::raw('ANY_VALUE(monitorings.idperiod) AS idperiod'), DB::raw('ANY_VALUE(monitorings.idgroup) AS idgroup'), DB::raw('ANY_VALUE(monitorings.idsubject) AS idsubject'), DB::raw('ANY_VALUE(monitorings.idnivel) AS idnivel'), DB::raw('ANY_VALUE(iduser) AS iduser'), 'monitoringcategories.name AS category', DB::raw('ANY_VALUE(monitoringcategorybyyears.percent) AS percent'), DB::raw("CONCAT('[',GROUP_CONCAT(CONCAT('{','\"monitoringtype\"',':','\"',REPLACE(monitoringtypes.name, '\"' , \"'\" ),'\"',',\"rating\"',':','\"',rating,'\"','}') ORDER BY monitoringtypes.name ),']') AS details"), DB::raw("ROUND(AVG(ANY_VALUE(monitorings.rating)) * ANY_VALUE(monitoringcategorybyyears.percent), 2) AS average"))
